...
- **Task 15: Semantic adjacent-skill matching — the embeddings are finally used**
  ...
  - **Scoring**: an adjacent pair earns fractional coverage credit equal to its cosine similarity — never more than an exact match, so rankings stay monotonic and explainable. Verified live on `gemini-embedding-001`: JD `Postgres` vs resume `PostgreSQL` → adjacent @ 83.98%; `Kubernetes` vs `K8s` → adjacent @ 87.61%; unrelated skills unaffected.
  - **Cost/latency**: at most **one `embed_batch` call per match computation** (all distinct unmatched strings batched and deduped), plus a bounded process-local cache keyed by normalized skill string — common skills (`"python"`) embed once per worker lifetime. Embedding-provider failures degrade gracefully to exact-only with a loud `semantic_fallback_embed_failed` log; a match never fails because the fallback did.
  - **Data model**: no schema migration — `matched_skills` is JSONB and entries gain additive keys: `match_type: "exact" | "adjacent"` (+ `similar_to`, `similarity` on adjacent). Legacy rows without `match_type` read as exact everywhere (API default, UI, CSV). Production verified at `alembic current == heads` (`c41f9a7de208`).
  - **Transparency everywhere**: `MatchedSkill` response schema exposes the new fields; the LLM rationale labels adjacent skills ("adjacent to X, ~84% similar"); results page rows/drawer render blue `≈ skill (N% similar)` pills (new `.tag-adjacent` + `--accent-adjacent*` design tokens — same pill recipe as `tag-evidence`/`tag-gap`: green=exact, blue=adjacent, gold=gap); candidate-detail cards carry an explicit "not a verbatim keyword match" note; both CSV exports annotate adjacency.
  - **Bug fix found en route**: server CSV export joined dict skill entries into the CSV (`TypeError` → 500 whenever a candidate had any matched skill); now formats entries explicitly.
...
- **Task 13: Ranked Results bulk actions, search & URL-persisted filters (+ job deletion)**
  ...
  - **URL persistence**: `min_score` and `q` (search term) live in URL query params — initialised from `useSearchParams()`, written back via debounced (250ms) `router.replace(..., { scroll: false })`. Navigating to a candidate dossier and back restores filter/search state. Page wrapped in `<Suspense>` as required by Next.js 16. Existing `api.getExportUrl()` path re-verified unchanged (reads the same derived state).
  - **Job deletion**: new org-scoped endpoint `DELETE /api/jobs/{job_id}` (cross-org IDs 404 like non-existent, per house pattern). Cascades to the job's resumes, match results, and batch jobs via DB-level `ondelete="CASCADE"` FKs; writes a DPDP audit event (`job_deleted`). Dashboard job cards gain a delete button with confirmation dialog and per-job pending spinner (`api.deleteJob()`).
  - **chore(frontend)**: fixed ESLint flat config for `eslint-config-next@16` — the default export is now the flat-config array and `eslint-config-next/core-web-vitals` is the documented subpath import; the old `next.coreWebVitals` shape crashed every lint run.
- **Task 13b: Persisted shortlist/reject status**
  - **Migration** `b4e8f2a19c73`: adds `match_results.status` (`String(20)`, NOT NULL, server default `'pending'`; values `pending | shortlisted | rejected`). No RLS changes needed (column on an already-scoped table).
  - **Endpoint**: `PATCH /api/jobs/{job_id}/matches/status` with `{match_ids: [...], status}` — org/job scoped (404 cross-org, matches restricted to this job), returns `{updated, status}`.
  - **Serialization**: `MatchResponse` now carries `status`; `pipeline.to_match_response` includes it.
  - **UI**: bulk action bar gains **Shortlist Selected** / **Reject Selected** buttons (per-button spinner, inline error line); rows show Shortlisted/Rejected badges; optimistic local update after a successful PATCH; selection clears on success.
